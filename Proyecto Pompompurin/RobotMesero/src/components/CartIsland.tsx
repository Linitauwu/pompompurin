...
function CartIsland() {
  ...
  const { carrito, aumentarCantidad, disminuirCantidad, eliminarDelCarrito, cantidadProductos, subtotal, impuestos, total, crearPedido, limpiarCarrito } = useCart();
  ...
  const enviarPedido = async () => {
    ...
    try {
      ...
      if (!user) {
        Alert.alert('Inicia sesión', 'Necesitas iniciar sesión antes de enviar un pedido.');
        return;
      }

      const pedidoLocal = crearPedido();
      const { data: pedido, error: pedidoError } = await supabase
        ...
        .insert({
          ...
          impuestos: pedidoLocal.impuestos,
          total: pedidoLocal.total,
          estado: 'pendiente',
        })
        .select('id, fecha, subtotal, impuestos, total, estado')
        .single();

      if (pedidoError || !pedido) throw pedidoError ?? new Error('No se creó el pedido.');

      const { error: itemsError } = await supabase.from('pedido_items').insert(
        pedidoLocal.items.map(item => ({
          pedido_id: pedido.id,
      ...
      agregarPedido({ ...pedidoLocal, id: pedido.id, fecha: pedido.fecha });
      limpiarCarrito();
      setAbierto(false);
      Alert.alert('Pedido enviado', 'La cocina ya recibió tu pedido.');
    } catch (error) {
      console.error('Error al guardar el pedido:', error);
      Alert.alert('No se pudo enviar', 'Revisa tu conexión y vuelve a intentarlo.');
    } finally {
      setEnviando(false);
    }
  };
  ...
  return (
    <View style={[styles.island, abierto && styles.islandAbierta]}>
      <TouchableOpacity style={styles.header} activeOpacity={0.85} onPress={() => setAbierto(!abierto)}>
        ...
        <View style={styles.totalContainer}><Text style={styles.totalLabel}>Total</Text><Text style={styles.total}>${total.toFixed(2)}</Text></View>
        <Text style={styles.arrow}>{abierto ? '▼' : '▲'}</Text>
      </TouchableOpacity>

      {abierto && <View style={styles.content}>
        <View style={styles.separator} /><Text style={styles.orderTitle}>🍮 Tu pedido</Text>
        {carrito.length === 0 ? <View style={styles.emptyContainer}>
          <Image source={require('./assets/gifs/pom-pom-purin-sleeping.gif')} style={styles.sleepingPurin} resizeMode="contain" />
          <Text style={styles.emptyText}>Tu pedido está vacío</Text><Text style={styles.emptySubtext}>¡Agrega algo rico! ♡</Text>
        </View> : <ScrollView style={styles.itemsContainer} showsVerticalScrollIndicator={false}>
          {carrito.map(item => <View key={item.producto.id} style={styles.item}>
        ...
        {carrito.length > 0 && <View style={styles.summary}>
...
const styles = StyleSheet.create({
  island: { position: 'fixed', bottom: 0, left: 20, right: 20, backgroundColor: '#F2CE72', borderTopLeftRadius: 32, borderTopRightRadius: 32, borderWidth: 3, borderBottomWidth: 0, borderColor: '#8A5A32', zIndex: 999999, elevation: 20, overflow: 'hidden' },
